...
class Attention(nn.Module):
    ...
    def forward(self, decoder_states, encoder_output, x_padding_masks):
        ...
        # (batch_size, seq_length)*(batch_size, seq_len)
        ...
        # (batch_size, 1, 2*hidden_units) = (batch_size fixed, (1, seq_length)*(seq_lenth, 2*hidden_units))
...
class Seq2seq(nn.Module):
    ...
    def forward(self, x, x_len, y, len_oovs, batch):
        ...
        """
        # replace oov's token as <UNK> token in x.
        # 创造一个和x形状一样的全是oov_token的以便下一步torch.where
        oov_token = torch.full(x.shape, self.v.UNK).long().to(self.DEVICE)
        # 和x形状一样，但按条件判断选用oov_token或者原x
        ...
        # encoder_states = 2 * (2, batch_size, hidden_units)
        encoder_output, encoder_states = self.encoder(x_copy)
        # Reduce encoder hidden states.
        # decoder_states = 2 * (1, batch_size, hidden_units)
        ...
        for t in range(y.shape[1]-1):
            # y = (batch_size, y_len)
            # decoder_input_t, decoder_target_t = (batch, 1)
            ...
            p_vocab, decoder_states = self.decoder(decoder_input_t.unsqueeze(1), decoder_states, encoder_output,
                context_vector)
