...
#     @timer('best k')
    def best_k(self, beam, k, encoder_output, x_padding_masks):
        ...
        context_vector, attention_weights = self.model.attention(beam.decoder_states, encoder_output, x_padding_masks)

        # Replace the indexes of OOV words with the index of UNK token
        # to prevent index-out-of-bound error in the decoder.
        decoder_input_t = self.replace_oov(decoder_input_t)
        p_vocab, decoder_states = self.model.decoder(decoder_input_t, beam.decoder_states, encoder_output, context_vector)
        ...
        if len(beam.tokens)==1:
            forbidden_ids = [self.vocab[u'这'],self.vocab[u'此'],self.vocab[u'采用'],self.vocab[u'，'],self.vocab[u'。'],self.vocab.UNK]
        ...
        best_k = [beam.extend(x, log_probs[x], decoder_states, attention_weights, beam.max_oovs, beam.encoder_input) for x in topk_idx.tolist()]
